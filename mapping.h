...
class Mapper
{
    public:
        ...
        void updateMapFeature(byte feature, double y, double x);
        byte readMapFeature(int y, int x);
        byte readMapFeature(double y, double x);
...
void Mapper::updateMapFeature(byte feature, double y, double x) {
  updateMapFeature( feature, (int)y, (int)x );  
}

byte Mapper::readMapFeature(double y, double x) {
  return readMapFeature( (int)y, (int)x );  
}
...
byte Mapper::readMapFeature(int y, int x) {
    int x_index = poseToIndex(x, MAP_X, MAP_RESOLUTION);
    int y_index = poseToIndex(y, MAP_Y, MAP_RESOLUTION);  
    int eeprom_address = (x_index * MAP_RESOLUTION) + y_index;
